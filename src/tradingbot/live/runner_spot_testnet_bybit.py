# src/tradingbot/live/runner_spot_testnet_bybit.py
...
async def run_live_bybit_spot_testnet(
    symbol: str = "BTC/USDT",
    ...
    """
    WS spot testnet -> barras 1m -> BreakoutATR -> Risk -> BybitSpotAdapter (orden real testnet)
    """
    ws = BybitSpotWSAdapter()
    exec_adapter = BybitSpotAdapter()
    agg = BarAggregator()
    strat = BreakoutATR()
    ...
    async for t in ws.stream_trades(symbol):
        ...
        if halted:
            ...
            if engine is not None:
                try:
                    insert_risk_event(engine, venue="bybit_spot_testnet", symbol=symbol,
        ...
        persist_bar_and_snapshot(engine, venue="bybit_spot_testnet", symbol=symbol, bar=closed, cur_pos=None)
