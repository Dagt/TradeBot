...
class MeanReversion(Strategy):
    ...
    def __init__(self, **kwargs):
        ...
        min_hold_param = kwargs.get("min_hold_bars")
        if min_hold_param is None:
            if tf_minutes <= 5.0:
                min_hold_param = 2.0
                ...
                min_hold_param = 0.0
        try:
            min_hold_param = float(min_hold_param)
        except (TypeError, ValueError):
            min_hold_param = 0.0
        if min_hold_param > 0:
            base_min_hold = int(math.ceil(min_hold_param))
        else:
            base_min_hold = 0
        self._base_min_hold_bars = base_min_hold
        self._min_hold_floor = 1 if tf_minutes <= 30.0 else 0

        max_hold_param = kwargs.get("time_stop", kwargs.get("max_hold_bars", 0.0))
        try:
            max_hold_param = float(max_hold_param)
        except (TypeError, ValueError):
            max_hold_param = 0.0
        if max_hold_param > 0:
            base_max_hold = int(math.ceil(max_hold_param))
            base_max_hold = max(1, base_max_hold)
        else:
            base_max_hold = 0
        self._base_max_hold_bars = base_max_hold
        self.time_stop = 0
    ...
    def _scaled_min_hold_bars(self, tf_minutes: float) -> int:
        ...
        if (
            ...
            return max(0, self._min_hold_floor)
        desired_minutes = self._base_min_hold_bars * base_minutes
        scaled_bars = int(math.ceil(desired_minutes / tf_minutes))
        min_bars = max(0, self._min_hold_floor)
        return max(min_bars, scaled_bars)

    def _scaled_max_hold_bars(self, tf_minutes: float) -> int:
        base_minutes = self._base_timeframe_minutes or tf_minutes
        if (
            self._base_max_hold_bars <= 0
            or tf_minutes <= 0
            or base_minutes is None
            or base_minutes <= 0
        ):
            return 0
        desired_minutes = self._base_max_hold_bars * base_minutes
        scaled_bars = int(math.ceil(desired_minutes / tf_minutes))
    ...
    def _maybe_time_stop_exit(
        ...
        *,
        ...
        if trade is None or max_hold_bars <= 0:
            return None
        if hold_bars < max(1, max_hold_bars):
            return None
        if isinstance(trade, dict):
            side_value = trade.get("side")
        else:
    ...
    def on_bar(self, bar: dict) -> Signal | None:
        ...
        max_hold_bars = self._scaled_max_hold_bars(tf_minutes)
        self.time_stop = max_hold_bars
        trade = None
        hold_bars = 0
        ...
        time_exit = self._maybe_time_stop_exit(
            bar,
            price,
            trade,
            hold_bars=hold_bars,
