...
class MeanReversion(StatefulStrategy):
    ...
    def on_bar(self, bar: dict) -> Signal | None:
        ...
        if self.pos_side is None:
            if last_rsi > self.upper and ofi_val <= 0 and price is not None:
                return self.open_position("sell", price)
            if last_rsi < self.lower and ofi_val >= 0 and price is not None:
